...
- [ ] Initialize project with custom package name `com.akashiverse.zenmath`.
- [ ] Configure GitHub Actions CI/CD for automated cloud builds.
- [ ] Setup `Riverpod` for state management and `go_router` for navigation.
- [ ] Integrate `Isar` database for local offline storage.
- [ ] Define `ThemeData` with "Midnight Zen" `ColorScheme`.
- [ ] Setup `TextTheme` hierarchy using `Outfit` and `Plus Jakarta Sans` (`google_fonts`).
- [ ] Create design token constants (`ZenColors`, `ZenSpacing`, `ZenRadii`).
- [ ] Build base wrapper widgets (`ZenScaffold`, `ZenCard`, `GlassBottomSheet`).
